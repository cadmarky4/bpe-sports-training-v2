...
switch ($action) {
	...
	case 'get_Categories_Select_rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY':  // SELECT
		
		$options = '<select>';
		if ($action == 'get_Categories_Select_rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY') {
			$options .= '<option value="0"></option>'; //rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY
		}
			...
			function buildCategorySelect(mixed $parent, mixed $categories_array, string $action, int $level):string {
				...
				if (isset($categories_array['parent_categories'][$parent])) {
					foreach ((array)$categories_array['parent_categories'][$parent] as $cat_id) {
						...
						$dis = (($action=='get_Categories_Select_rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY' AND $level==3) ? ' disabled' : '');
