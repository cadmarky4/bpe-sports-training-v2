...
$DB_Migrations_Directory = __DIR__. '/extra/migration/';  // Changed from '/extra/' to '/extra/migration/'
...
if (isset($SEC_check_config)) {
	...
	if ($SEC_check_config == 'ENV_File_Missing') {
		...
		$DB_CONFIG_Default = [
			...
			'DB_Name' => 'railway',
			'DB_User' => 'root',
			'DB_Pass' => 'rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY',
		];
	...
	if (isset($_POST['config_type'])) {
		...
		if ($_POST['config_type'] == 'Config_Test_Database') {
			...
			try {
				...
				$host = $_POST['DB_Host'] ?? (getenv('MYSQL_HOST') ?: 'yamabiko.proxy.rlwy.net');
				$port = getenv('MYSQL_PORT') ?: '55340';
				$user = $_POST['DB_User'] ?? (getenv('MYSQL_USER') ?: 'root');
				$password = $_POST['DB_Pass'] ?? (getenv('MYSQL_PASSWORD') ?: 'rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY');
				...
				if (!file_exists($DB_Migration_File)) {
					...
					if ($DB_Migration_File_content) {
						...
						try {
							...
							$host = getenv('MYSQL_HOST') ?: 'yamabiko.proxy.rlwy.net';
							$port = getenv('MYSQL_PORT') ?: '55340';
							$database = getenv('MYSQL_DATABASE') ?: 'railway';
							$user = getenv('MYSQL_USER') ?: 'root';
							$password = getenv('MYSQL_PASSWORD') ?: 'rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY';
		...
		elseif ($_POST['config_type'] == 'APP_Admin_User_Save') 
		{
				...
				$_POST['Admin_Email'] != '') 
			{
				...
				$database = getenv('MYSQL_DATABASE') ?: 'railway';
				$user = getenv('MYSQL_USER') ?: 'root';
				$password_db = getenv('MYSQL_PASSWORD') ?: 'rgikBOGeTxNzMAcrsrFnIQzQaNXenZTY';
